...
if (($aksi=='edit') && (get_option('profil_dosen')=='ya')) {
	echo '<div class="card">
		...
		elv_edituser($profil_dosen,$current_id);
	echo '</div></div>';
} if (($aksi=='editakun') && (get_option('akun_profil_dosen')=='ya')){
	echo '<div class="card">
		<div class="card-header bg-info text-white font-weight-bold">
		...
		<div class="card-body pb-5">';
		elv_edit_akun_user($profil_dosen,$current_id);
	echo '</div></div>';	
} if(empty($aksi)) {
	elv_lihatuser($profil_dosen,$current_id);
	if (get_option('profil_dosen')=='ya'){
		echo '<a class="btn btn-info btn-sm me-1" href="?halaman=profil&aksi=edit"><i class="fa fa-pencil"></i> Edit Profil</a>';
	} if (get_option('akun_profil_dosen')=='ya'){
