...
echo '<div id="elv-main" class="row">';
	...
	echo '<div id="elv-sidebar" class="col-md-3"><div class="card pt-sm-4 pb-sm-5">';
		...
		$array_menu = array (
			...
			'menu' => array (
				...
				array('judul' => 'KHS', 'halaman' => 'khsdosen'),
	...
	echo '<div id="elv-contain" class="col-md-9">';
		...
		if(empty($halaman)) {
			...
			elv_lihatuser($profil_dosen,$user_id);
		} if($halaman=='profil') {
			echo '<h5 class="'.$class.'"> Profil </h5>';
			require_once ( VELOCITY_SIA_PLUGIN_DIR.  '/inc/dosen/profile.php' );
		} if($halaman=='khsdosen') {
			echo '<h5 class="'.$class.'"> KHS </h5>';
			require_once ( VELOCITY_SIA_PLUGIN_DIR.  '/inc/dosen/khs-dosen.php' );
		} if($halaman=='tambahkhs') {
			echo '<h5 class="'.$class.'"> Input Nilai KHS </h5>';
