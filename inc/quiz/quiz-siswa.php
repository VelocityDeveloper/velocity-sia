...
if($aksi) {
  ...
  WHERE $table_makul.tahun_akademik = $thn_ajaran AND $table_makul.semester = $id_semester AND $table_makul.id_prodi = $prodiku AND $table_krs.id_mahasiswa = $user_id");
	...
	if ($get_jadwal) {
		...
		//ambil id quiz yang sudah dikerjakan
		$sudahjawab 		= $wpdb->get_results("SELECT * FROM $table_quiz WHERE tipe = 'jawab' and iduser = $user_id ");
		if ($sudahjawab) {
			$idquizk				= array();
		  foreach ($sudahjawab as $sd) {
			...
			if ($set) {
				$idquizk = array_map(function($v) {
				    return "'%" . esc_sql($v) . "%'";
				}, $idquizk);
				$idquizk = implode(' or id like ', $idquizk);
			} else {
				$idquizk = array_map(function($v) {
						return "'" . esc_sql($v) . "'";
				}, $idquizk);
				$idquizk = implode(' and id != ', $idquizk);
			}
		} else {
			$idquizk = '0';
		}
	  ...
	  foreach ($get_jadwal as $jadwal) {
		...
		if ($set) {
			//tampil quiz sudah dikerjakan
			$exist 				= $wpdb->get_var("SELECT COUNT(*) FROM $table_quiz id WHERE tipe = 'publish' and ( tujuan like $idmatakul ) and (tujuan like '%$kelasku%') and ( id like $idquizk )");
			$tampil_quiz 	= $wpdb->get_results("SELECT * FROM $table_quiz WHERE tipe = 'publish' and ( tujuan like $idmatakul ) and (tujuan like '%$kelasku%') and ( id like $idquizk ) ORDER BY tanggal DESC LIMIT ${offset}, ${items_per_page}");
		} else {
			$exist 				= $wpdb->get_var("SELECT COUNT(*) FROM $table_quiz id WHERE tipe = 'publish' and ( id != $idquizk ) and ( tujuan like $idmatakul ) and (tujuan like '%$kelasku%') ");
			$tampil_quiz 	= $wpdb->get_results("SELECT * FROM $table_quiz WHERE tipe = 'publish' and ( id != $idquizk ) and ( tujuan like $idmatakul ) and (tujuan like '%$kelasku%') ORDER BY tanggal DESC LIMIT ${offset}, ${items_per_page}");
		}
	?>

			<div class="btn-group" role="group">
			  <a class="btn <?php if ($set) { echo 'btn-secondary';} else { echo 'btn-info';}?>" href="?halaman=quiz">Belum Dikerjakan</a>
				<a class="btn <?php if ($set) { echo 'btn-info';} else { echo 'btn-secondary';}?>" href="?halaman=quiz&set=sudah">Sudah Dikerjakan</a>
			</div>
	      ...
	      ?>
	        			<tr class="tr-<?php echo $id; ?> tr-quiz">
